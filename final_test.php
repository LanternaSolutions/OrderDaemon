<?php
/**
 * Final test to verify the timestamp fix works
 */
...
foreach ($test_payloads as $index => $payload) {
    ...
    echo "Expected Date: " . gmdate('Y-m-d H:i:s', (int)$payload['ts']) . "\n";
...
if ($all_success) {
    ...
    echo "Each component will now display its own timestamp from the payload.\n";
} else {
