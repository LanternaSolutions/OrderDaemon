...
echo "<p>Log table ({$logTable}): " . ($logTableExists ? 'EXISTS' : 'MISSING') . "</p>\n";
echo "<p>Payload table ({$payloadTable}): " . ($payloadTableExists ? 'EXISTS' : 'MISSING') . "</p>\n";
...
if (empty($checkoutEvents)) {
    ...
    foreach ($sampleEvents as $event) {
        echo "<li>{$event->event_type} ({$event->count} events)</li>\n";
    }
    echo "</ul>\n";
    
} else {
    echo "<p><strong>Found " . count($checkoutEvents) . " checkout events:</strong></p>\n";
    
    foreach ($checkoutEvents as $i => $event) {
        echo "<hr>\n";
        echo "<h3>Event #" . ($i + 1) . " (ID: {$event->id})</h3>\n";
        ...
        echo "<li>Timestamp: {$event->timestamp}</li>\n";
        echo "<li>Payload ID: {$event->payload_id}</li>\n";
        echo "<li>Process ID: {$event->process_id}</li>\n";
        echo "</ul>\n";
        
        // Analyze payload structure
        echo "<p><strong>Payload Analysis:</strong></p>\n";
        if (empty($event->payload)) {
            echo "<p>❌ <strong>Payload is empty</strong></p>\n";
        } else {
            echo "<p>✅ <strong>Payload exists</strong> (length: " . strlen($event->payload) . " characters)</p>\n";
            
            // Try to decode JSON
            $payloadData = json_decode($event->payload, true);
            if (json_last_error() !== JSON_ERROR_NONE) {
                echo "<p>❌ <strong>Invalid JSON:</strong> " . json_last_error_msg() . "</p>\n";
                echo "<pre>" . htmlspecialchars(substr($event->payload, 0, 500)) . "...</pre>\n";
            } else {
                echo "<p>✅ <strong>Valid JSON payload</strong></p>\n";
                ...
                echo "<p><strong>ProcessLogger format:</strong> " . ($isProcessLogger ? 'YES' : 'NO') . "</p>\n";
                ...
                echo "<p><strong>UniversalEvent indicators:</strong></p>\n";
                echo "<ul>\n";
                echo "<li>Has eventData/event_data: " . ($hasEventData ? 'YES' : 'NO') . "</li>\n";
                echo "<li>Has rawData: " . ($hasRawData ? 'YES' : 'NO') . "</li>\n";
                echo "<li>Has eventType/event_type: " . ($hasEventType ? 'YES' : 'NO') . "</li>\n";
                echo "</ul>\n";
                
                if ($hasRawData) {
                    echo "<p><strong>rawData keys:</strong> " . implode(', ', array_keys($payloadData['rawData'])) . "</p>\n";
                    echo "<p><strong>rawData empty:</strong> " . (empty($payloadData['rawData']) ? 'YES' : 'NO') . "</p>\n";
                }
                
                // Show structure sample
                echo "<h4>Payload Structure Sample:</h4>\n";
                echo "<pre>";
        ...
        if (!empty($payloadData)) {
            echo "<h4>Current Extractor Test:</h4>\n";
            ...
            foreach ($components as $j => $component) {
                echo "<p><strong>Component $j:</strong></p>\n";
                echo "<ul>\n";
...
echo "<p>This analysis will help determine the correct extraction logic needed.</p>\n";
