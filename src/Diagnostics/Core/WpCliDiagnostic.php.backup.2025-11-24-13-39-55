...
class WpCliDiagnostic extends AbstractDiagnostic
{
    ...
    {
        return __('admin.diagnostics.test.wp_cli.name', 'order-daemon');
    }
    ...
    {
        return __('admin.diagnostics.test.wp_cli.description', 'order-daemon');
    }
    ...
    protected function execute(): DiagnosticResult
    {
        ...
        if (!function_exists('shell_exec')) {
            return DiagnosticResult::failure(
                __('admin.diagnostics.test.wp_cli.failure.title', 'order-daemon'),
                __('admin.diagnostics.test.wp_cli.failure.shell_exec_disabled', 'order-daemon'),
                [
                    'shell_exec_available' => false,
                    'disabled_functions' => ini_get('disable_functions'),
                ],
                [__('admin.diagnostics.test.wp_cli.failure.shell_exec_recommendation', 'order-daemon')]
            );
        }
        ...
        if (empty($wp_cli_output)) {
            return DiagnosticResult::failure(
                __('admin.diagnostics.test.wp_cli.failure.title', 'order-daemon'),
                __('admin.diagnostics.test.wp_cli.failure.explanation', 'order-daemon'),
                [
                ...
                [__('admin.diagnostics.test.wp_cli.failure.recommendation', 'order-daemon')]
            );
        }
        ...
        if (strpos($wp_cli_output, 'WP-CLI') === false) {
            return DiagnosticResult::failure(
                __('admin.diagnostics.test.wp_cli.failure.title', 'order-daemon'),
                __('admin.diagnostics.test.wp_cli.failure.invalid_output', 'order-daemon'),
                [
                    'command_tested' => 'wp --version',
        ...
        $result = DiagnosticResult::success(
            __('admin.diagnostics.test.wp_cli.success.title', 'order-daemon'),
            /* translators: %s: WP-CLI version information */
            sprintf(__('admin.diagnostics.test.wp_cli.success.details', 'order-daemon'), trim($wp_cli_output)),
            $details
        );

        // Add warning if WP-CLI can't access WordPress
        if (!$can_access_wp) {
            $result->addRecommendation(__('admin.diagnostics.test.wp_cli.warning.wp_access', 'order-daemon'));
        }
