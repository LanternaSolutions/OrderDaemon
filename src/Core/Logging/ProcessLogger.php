...
final class ProcessLogger
{
    ...
    public function finish(string $final_status, string $summary)
    {
        ...
        try {
            ...
            $envelope = [
                ...
                'payload_components' => $this->components,
            ];

            // NEW: Merge deferred context into components before writing
            foreach ($this->components as &$component) {
                if (isset($component['key']) && isset($this->deferred_context[$component['key']])) {
                    $component['data'] = array_merge(
                        is_array($component['data']) ? $component['data'] : [],
                        $this->deferred_context[$component['key']]
                    );
                }
            }
            unset($component); // break reference
            // Update envelope with merged components
            $envelope['payload_components'] = $this->components;
            // Clear deferred context to free memory
            $this->deferred_context = [];

            // Canonical single-write via AuditTrailLogger
            $data = [
                'order_id'   => $this->context['order_id'],
                'details'    => $envelope,
                'process_id' => $this->correlation_id,
                'log_category' => 'process',
                'source'     => $final_source,
            ];

            return AuditTrailLogger::record(
    ...
    {
        $short = substr(preg_replace('/[^a-z0-9]+/', '', $type), 0, 4);
        $primary = $order_id ?: 'na';
        return sprintf('odcm:%s:%s:%s', $short, (string)$primary, wp_generate_uuid4());
    }
    ...
    {
        return sanitize_key($kind) . '-' . wp_generate_uuid4();
    }
